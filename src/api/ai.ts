 ...
 */
router.post('/moderate-content', authenticateToken, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/moderate-game', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const game = await Game.findById(gameId).populate('players', 'username avatar role isAlive');
 ...
 */
router.post('/analyze-behavior', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    // Fetch game history for the player
    const gameHistory = await Game.getGameHistory(new Types.ObjectId(playerId), 20);
    ...
    const analysis = await AIService.analyzePlayerBehavior(player, gameHistory, currentGame);
 ...
 */
router.post('/gameplay-tips', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const isPlayerInGame = game.players.some((p: any) => p._id.equals(playerId));
    if (!isPlayerInGame) {
    ...
    const tips = await AIService.provideGameplayTips(player, game, experience);
 ...
 */
router.post('/chat-assistance', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    // Fetch the message
    const message = await ChatMessage.findById(messageId).populate('playerId', 'username');
    if (!message) {
      return res.status(404).json({
    ...
    if (action && moderationResult.suggestedAction) {
      switch (action) {
        case 'moderate':
          await message.moderate('AI-assisted moderation');
          break;
        case 'approve':
